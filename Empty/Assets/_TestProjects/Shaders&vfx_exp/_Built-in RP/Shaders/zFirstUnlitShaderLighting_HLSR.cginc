...
Interpolators vert(MeshData v)
{
    ...
    //zero heighest lod level// *2 increase heigh -1 press down
...
float4 frag(Interpolators i) : SV_Target
{
    ...
    //Texture
    float4 albedo = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, i.uv); //!SAMPLE_TEXTURE2D
    ...
    col = lerp(_ColorBase, _ColorSecond, saturate(pulse)); //!saturate(pulse)
    //clip(col - 0.001);
    ...
    float3 lightColor = _MainLightColor.xyz * 1.00; //!_MainLightColor
    ...
    specularLight *= _MainLightColor.xyz;
